...
for seed in seeds :
    ...
    print " Analysis | miscellaneous "
    main_dir = 'analysis_seed_%d_stime_0.5_hanning' % seed
    os.chdir( workdir )
    ...
    os.system( './x_analyse.py --do_csd --do_psd setup_analysis.pkl' )
    os.chdir( workdir )


    print 'Waiting for jobs of miscellaneous analyses to finish...'
    jobidpaths = glob.glob( main_dir+'/x_pklTStoCSD*jobid' ) + glob.glob( main_dir+'/x_pklTStoPSD*jobid' )
    jobids = []
    ...
    while True :
        ...
        print 'jobxits' , jobxits
        if jobxits.count(1) == len(jobids) :
            print 'Miscellaneous analyses have finished.'
            break
        else :
            time.sleep( 20 )
            continue
    ...
    os.chdir( workdir )
    os.chdir( merge_dir )
    os.system( 'rm -r *.out *.sub' ) ; os.chdir( workdir )
