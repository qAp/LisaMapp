...
"""
This is like AnisotropySearch.py, but for A and E only.  T is not used.
"""
...
def coarsegrain(data,flowx,deltafx,flowy,deltafy,Ny):
    ...
    coarsedata = np.zeros( (data.shape[0],Ny) , complex )
...
class TimeSeries(object):
    ...
    t -- time 
    A -- time-series A
    E -- time-series E ( all the above 3 attibutes are Coarsables )
    Methods:
    ...
    def __init__(self,tsdict):
        ...
        Positional parameter:
        tsdict -- dictionary containing t,A,E
...
#20110205: self.psd() and self.csd() removed because matplotlib/pylab cannot be imported on the computing nodes of the cluster.
    ...
    def CrossSpectraData(self,xstr,ystr,window,fftlength,coarsable=False):
        ...
        Positional parameters:
        xstr -- observable, either 'A' or 'E' 
        ystr -- observable, either 'A' or 'E' 
...
class XXX_test(object):
    ...
    def getCrossSpectraData(self,window=None,fftlength=None):
        f = self.csd['PowerSpectra']['f']
        self.csd['PowerSpectra'][ 'P'+self.pair ].data #*= 1e42
        C = self.csd['PowerSpectra'][ 'P'+self.pair ]
        self.fcsdata , self.csdata = f , C
        return f , C
...
class TheZs( object ):
    def __init__( self , N=10 , seed=None ):
        ...
        np.random.seed( seed )
        p = [ np.random.standard_normal( (N,) ) for i in range(2) ]
        q = [ np.random.standard_normal( (N,) ) for i in range(2) ]
...
def CSpectra_to_ShortTermFT( cspecdict , seed=None ):
    ...
    sdatas = [ np.zeros( (Nf,) , dtype='complex' ) for i in range(2) ]

    for k in range( Nf ):

        clist = [ cspec.data[k] for cspec in cspeclist ]
        cmatrix = np.array( [ [ clist[0]            , clist[1]   ] ,
                              [ np.conj( clist[1] ) , clist[2]   ] ] )
        ...
        s = ( Z.z1[k]*cmath.sqrt( np.real( w[0] ) )*V[:,0] + Z.z2[k]*cmath.sqrt( np.real( w[1] ) )*V[:,1] )

        sdatas[0][k] , sdatas[1][k] = np.conj( s )


    scoarsables = [ Coarsable( sdata , Offset1=fOffset , Cadence1=fCadence ) for sdata in sdatas ]    
    shortft = ShortTermFT( s1=scoarsables[0] , s2=scoarsables[1] )
...
def InverseFT( shortft , Neven=False , fNyq=False , tOffset=0 ):
    ...
    tsdict = {}
    tsdict['s1'] , tsdict['s2'] = tslist[0] , tslist[1]
